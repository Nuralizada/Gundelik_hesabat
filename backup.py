...
if page == "Current Month,Current Year":
    ...
    with col_start:
        st.markdown('<p class="date-label">Başlanğıc Tarixi</p>', unsafe_allow_html=True)
        start_date = st.date_input(' ', value=None, key='start_date')  # Tarixi boş qoyun

    with col_end:
        st.markdown('<p class="date-label">Bitiş Tarixi</p>', unsafe_allow_html=True)
        end_date = st.date_input(' ', value=None, key='end_date')  # Tarixi boş qoyun

    # Tarix aralığı seçildikdə məlumatları süzün
    if start_date and end_date:
        # Tarixləri formatlayın
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)

        # Filtrə əsasən məlumatların süzülməsi
        filtered_df = fakt_df[(fakt_df['Tarix'] >= start_date) & 
                              (fakt_df['Tarix'] <= end_date) & 
                              (fakt_df['Rejim'] == 'Tranzit') & 
                              (fakt_df['əsas_yüklər'] != 'Digər yüklər')]

        # "Malın adı" və "Göndərən ölkə"ya görə qruplaşdırma
        grouped_df = filtered_df.groupby(['əsas_yüklər', 'Göndərən ölkə', 'GSA']).agg({
            'Vaqon_sayı': 'sum',
            'Həcm_fakt': 'sum'
        }).reset_index()

        # Malın adını və göndərən ölkəni əlifba sırasına görə sıralamaq
        grouped_df = grouped_df.sort_values(by=['əsas_yüklər', 'Göndərən ölkə'], ascending=[True, True])
        ...
        # Plotly cədvəlini qurmaq
        fig = go.Figure(data=[go.Table(
            columnwidth=[250, 200, 200, 100, 100],
            header=dict(values=['Malın adı', 'Göndərən ölkə', 'Göndərən stansiya', 'Vaqon sayı', 'Həcm'],
                        ...
                        height=40,
                        line_color='white'),
            cells=dict(values=[grouped_df['əsas_yüklər'], grouped_df['Göndərən ölkə'], grouped_df['GSA'], 
                               grouped_df['Vaqon_sayı'].map('{:,.0f}'.format), 
                               grouped_df['Həcm_fakt'].map('{:,.0f}'.format)],
                     fill_color=[['#c4c1b9'] + ['#F0F2F6'] * (len(grouped_df) - 1)],
                     align='center',
                     font=dict(size=14, color=['Black'] + ['black'] * (len(grouped_df) - 1)),
                     height=30))
        ])

        # Cədvəli göstərmək
        fig.update_layout(height=1000, margin=dict(l=10, r=10, b=10, t=10))
        st.plotly_chart(fig, use_container_width=True)

        # Göstəriciləri hesabla
        toplam_plan = total_vaqon_sayi
